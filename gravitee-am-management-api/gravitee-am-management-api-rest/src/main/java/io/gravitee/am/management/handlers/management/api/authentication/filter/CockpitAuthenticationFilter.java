...
public class CockpitAuthenticationFilter extends GenericFilterBean {
    ...
    private UsernamePasswordAuthenticationToken convertToAuthentication(JWT jwt) {
        ...
        // Retrieve standard claims and add them to user's additional infos.
        HashMap<String, Object> additionalInfos = new HashMap<>();
        additionalInfos.put("preferred_username", jwt.get(StandardClaims.PREFERRED_USERNAME));
        additionalInfos.put("name", jwt.get(StandardClaims.NAME));
        additionalInfos.put("family_name", jwt.get(StandardClaims.FAMILY_NAME));
        additionalInfos.put("given_name", jwt.get(StandardClaims.GIVEN_NAME));
        additionalInfos.put("email", jwt.get(StandardClaims.EMAIL));

        user.setAdditionalInformation(additionalInfos);

        // Map role names from jwt to AM role ids and add them to user's roles.
        user.setRoles(convertToRoleIds((List<String>) jwt.get(CustomClaims.ROLES), organizationId));
